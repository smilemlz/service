...
class UserController extends CommonController {
    ...
    public function third_login() {
        ...
        $info = model('ClipsBase')->get_third_user_info($type);		//微信公众号登录插件的信息
        // 判断是否安装
        if (!$info) {
	...
	public function wxlogin(){
		if(!empty($_POST)){
			...
			$openid=$_POST["openid"];
			$sign=$_POST["sign"]; 		
			$userinfo = $token = json_decode(stripslashes($token),true);			//去除反斜杠之后转换成数组	
			...
			$userinfo['openid'] = $openid=$_REQUEST["openid"];
    ...
    public function take_list(){
        ...
        // 分页
        $filter['page'] = '{page}';
        $offset = $this->pageLimit(url('take_list', $filter), 5);
        $offset_page = explode(',', $offset);
        $count =  M()->table('user_gift_gard')->field('count(*)')->where("user_id='$this->user_id'")->getOne();
        $take_list = $this->get_user_take($this->user_id, $offset_page[1], $offset_page[0]);
        $this->assign('pager', $this->pageShow($count));
        $this->assign('take_list', $take_list);
    
        $this->assign('title', '礼品卡兑换列表');
    
        $this->display('user_take_list.dwt');
    }
    
    public function get_user_take($user_id, $num = 10, $start = 0) {
         
    
        $sql = "SELECT ub.*, u.user_name, u.email, o.goods_name, bt.gift_name ".
            " FROM ". $this->model->pre . "user_gift_gard AS ub ".
            " LEFT JOIN " . $this->model->pre . "gift_gard_type  AS bt ON bt.gift_id=ub.gift_id ".
            " LEFT JOIN " . $this->model->pre . "users AS u ON u.user_id=ub.user_id ".
            " LEFT JOIN " . $this->model->pre . "goods AS o ON o.goods_id=ub.goods_id WHERE ub.user_id='$this->user_id'".
            " ORDER BY ub.user_time DESC " .
            " LIMIT ". $start .", $num";
        $row =  $this->model->query($sql);
        foreach ($row as $key=>$val){
            $row[$key]['user_time'] = local_date(C('date_format'), $val['user_time']);
        }
     ...
     * 确定收货
     **/
    public function confim_goods(){
        $take_id = isset($_REQUEST['take_id']) ? intval($_REQUEST['take_id']) : 1;
    
        if($this->model->query("UPDATE " .$this->model->pre."user_gift_gard SET status='3' WHERE gift_gard_id = '$take_id'"))
        {
            ecs_header("Location: " . url('user/take_list')."\n");
            exit;
        }
        else
        {
            show_message('收货失败-_-，请重新收货', '返回重新收货', url('user/take_list'));
        }
    }
